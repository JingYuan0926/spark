...
export default async function handler(
  ...
  if (!amount) {
    return res
      .status(400)
      .json({ success: false, error: "amount is required (HBAR)" });
  }
  ...
  try {
    ...
    const hbarAmount = ethers.parseEther(String(amount)); // Hedera EVM relay uses 18 decimals (weibars)
    const tx = await vault.fundVault({ value: hbarAmount });
    ...
    return res.status(200).json({
      ...
      message: `Funded vault with ${amount} HBAR`,
    });
  } catch (error) {
